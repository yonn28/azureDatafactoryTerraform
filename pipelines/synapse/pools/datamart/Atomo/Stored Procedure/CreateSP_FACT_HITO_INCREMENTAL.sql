...
-- Description: Se crea el Store Procedure SP_FACT_HITO_INICIAL para el llenado incremental de la tabla ATOMO.DWH.FACT_HITO

CREATE PROC [ATOMO].[SP_FACT_HITO_INCREMENTAL] AS 
 SET NOCOUNT ON;
...
--WHERE CAST(A.[DTM_FECHACARGA] AS DATE) >= CONVERT(char(10), GetDate()-5,126)
),

CONFIGURACION_TABLERO AS(
SELECT DISTINCT FK_PARAM_INDICADOR, FK_INDICADOR, DESC_ANNIO
FROM [ATOMO].[STG.DIM_CONFIGURACION_TABLERO_LOG]
WHERE DESC_HITO_INDICADOR='Hito' AND DESC_SIGLA_AREA='ILP'),
--AND CAST([DTM_FECHACARGA] AS DATE) >= CONVERT(char(10), GetDate()-5,126)),
