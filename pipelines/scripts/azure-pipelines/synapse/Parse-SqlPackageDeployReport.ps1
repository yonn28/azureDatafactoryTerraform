...
if ($issues) {
    $issues | foreach {
        ...
        Write-Host "##vso[task.logissue type=error] " $_.Issue.Value
    }
    Write-Host "##vso[task.setvariable variable=foundDataIssues;isOutput=true]$true"
}
else {
    Add-Content -Path $OutputFilePath -Value " - No data issues where detected"
    Write-Host "##vso[task.setvariable variable=foundDataIssues;isOutput=true]$false"
}
...
if ($alerts) {
    $alerts | foreach {
        ...
        Write-Host "##vso[task.logissue type=warning] Potential harmful operation detected: " + $_.Name
    }
}
else {
